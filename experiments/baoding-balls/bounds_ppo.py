...
class BoundsPPO:
    ...
    def __init__(
        ...
        if not math.isfinite(bounds_coef) or not 0 <= bounds_coef <= .001:
            raise ValueError('Bounds coefficient outside tested range')
    ...
    def update(self) -> dict[str, float]:
        ...
        for (
            ...
            # Experiment extension to upstream RSL-RL v3.1.2. Preserve Gaussian
            # sampling/log-probabilities; regularize means directly, before clip.
            bounds_loss = (mu_batch.abs()-1.1).clamp_min(0).square().sum(-1).mean()
            loss = loss + self.bounds_coef * bounds_loss
        ...
        loss_dict = {
            ...
            "actor_bounds": mean_bounds_loss / num_updates,
        }
        if self.rnd:
